...
try:
    ...
    st.error("The dashboard could not find the required IPL data files.")
    st.caption("Please make sure the files named matches_clean.csv and deliveries_clean.csv are present in the data folder.")
    st.caption(f"Expected location: {DATA_DIR}")
    st.stop()
except Exception as exc:
    st.error("The dashboard could not load the IPL data.")
...
if not selected_seasons:
    st.sidebar.warning("Please select at least one season to view the dashboard.")
    st.info("Choose one or more seasons from the sidebar to start exploring the IPL data.")
    st.stop()

season_filtered_matches = matches[matches["season"].isin(selected_seasons)].copy()
available_teams = sorted(
    set(season_filtered_matches["team1"].dropna().tolist() + season_filtered_matches["team2"].dropna().tolist())
)
available_venues = sorted(season_filtered_matches["venue"].dropna().unique())

if "selected_teams" not in st.session_state:
    st.session_state.selected_teams = available_teams
if "selected_venues" not in st.session_state:
    st.session_state.selected_venues = available_venues

if not set(st.session_state.selected_teams).issubset(set(available_teams)):
    st.session_state.selected_teams = available_teams
if not set(st.session_state.selected_venues).issubset(set(available_venues)):
    st.session_state.selected_venues = available_venues
...
if not available_teams:
    st.sidebar.warning("No teams are available for the selected seasons. Try choosing a different season.")
elif not available_venues:
    st.sidebar.warning("No venues are available for the selected seasons. Try choosing a different season.")
...
# ============================================================
    ...
    st.subheader("Simple view: runs scored in each over")
    st.caption("This chart shows the average runs scored in one over, across the selected matches.")
    st.markdown("<div class='section-card'><strong>Easy meaning:</strong> each bar shows how many runs a typical over gave in your selected filter.</div>", unsafe_allow_html=True)

    if filtered_deliveries.empty:
        st.warning("No ball-by-ball data is available for the current filters. Please widen your season, team, or venue selection.")
        st.stop()

    over_runs = (
        filtered_deliveries.groupby(["match_id", "inning", "over"], as_index=False)["total_runs"]
        .sum()
        .groupby("over", as_index=False)["total_runs"]
        .mean()
        .rename(columns={"total_runs": "avg_runs_per_over"})
    )

    estimated_total_runs = round(float(over_runs["avg_runs_per_over"].mean()) * 20, 1)
    st.info(f"Approximate total runs in a full 20-over innings: {estimated_total_runs}")
    ...
    ax.bar(over_runs["over"], over_runs["avg_runs_per_over"], color=colors, edgecolor="white")
    ax.set_title("Average runs in one over", fontsize=13)
    ax.set_xlabel("Over")
    ax.set_ylabel("Average runs")
    max_value = max(12.0, float(over_runs["avg_runs_per_over"].max()) + 2.0)
    ax.set_ylim(0, max_value)
    ax.grid(axis="y", alpha=0.25)
    plt.tight_layout()
    st.pyplot(fig, use_container_width=True)

    st.subheader("Simple view: toss choices")
    ...
    with col_left:
        ...
        ax.set_title("How teams choose after winning the toss", fontsize=12)
        plt.tight_layout()
        st.pyplot(fig, use_container_width=True)
